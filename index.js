...
client.on('ready', async () => {
  ...
  client.user.setActivity(`SA-SHOP ❤️`)
});
client.on("message", async(message) =>{
  ...
  if (command == prefix + 'send' || command == prefix + 'ticket') {
    ...
    let button7 = new disbut.MessageButton()
      .setStyle(`green`)
      .setEmoji(`🔑`)
      .setLabel(`Aldığım Ürünler İçin Sorular`)
      .setID("oyb")
  ...
  if (button.id === "oyb"){
    ...
    button.guild.channels.create(`ticket-${button.clicker.user.username}`, {
      permissionOverwrites: [
        ...
        ], parent: (await db.get(`Channels_${button.message.guild.id}.Cat`)), position: 1, topic: `A Ticket : <@!${button.clicker.user.id}>`, reason: "Tüm hakları SilentA'ya aittir."
      }).then(async channel => {
        channel = channel
